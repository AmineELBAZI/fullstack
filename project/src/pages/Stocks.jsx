...
const Stocks = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Simuler réapprovisionnement en mettant à jour la quantité en mémoire
    const produit = allProduits.find(p => p.id === formData.produitId);
    if (produit) {
      produit.quantityStock += formData.quantite;
      toast.success('Réapprovisionnement simulé avec succès');
      handleCloseModal();
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          {/* Stock Normal */}
          {['Normal', 'Faible', 'Rupture'].map((statusName, idx) => {
            const produits =
              statusName === 'Normal' ? produitsNormal :
              statusName === 'Faible' ? produitsFaible :
              produitsRupture;
            const textColor =
              statusName === 'Normal' ? 'text-green-700' :
              statusName === 'Faible' ? 'text-amber-700' :
              'text-red-700';

            return (
              <div key={statusName}>
                <h3 className={`text-lg font-bold mb-4 ${textColor}`}>Stock {statusName}</h3>
                {produits.length === 0 && (
                ...
                <div className="max-h-96 overflow-y-auto hide-scrollbar space-y-4">
                  {produits.map(produit => {
                    ...
                    return (
                      <div
                        key={produit.id}
                        className={`bg-${status.label === 'Normal' ? 'green' : status.label === 'Faible' ? 'amber' : 'red'}-50 hover:bg-white rounded-2xl shadow-lg border border-${status.label === 'Normal' ? 'green' : status.label === 'Faible' ? 'amber' : 'red'}-200 p-6 hover:shadow-xl transition-shadow duration-300`}
                      >
                        <div className="flex items-center justify-between mb-4 border-b border-black-200 pb-2">
                          <h3 className={`text-lg font-bold truncate ${status.color.split(' ')[0]}`}>{produit.name}</h3>
                          <span className={`text-xs px-3 py-1 rounded-full text-black font-medium ${status.color}`}>{status.label}</span>
                        </div>
